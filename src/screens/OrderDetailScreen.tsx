// ─── OrderDetailScreen — Calculadora de Lucro (Regra de Negócio) ─────────────
// Esta é a tela mais importante do app:
//   • O motorista vê o breakdown COMPLETO: Bruto → Taxa Muvv → Líquido
//   • A taxa varia DINAMICAMENTE conforme a categoria selecionada
//   • Para fretes ZPE, exibe o equivalente em Euros
//
// Lógica de taxas: ver src/utils/calculations.ts → MUVV_RATES
...
export function OrderDetailScreen({ freight }: OrderDetailScreenProps) {
  // Categoria e valor bruto são editáveis para simulação de lucro
  ...
  // Recalcula automaticamente toda vez que categoria ou valor muda
  const { gross, fee, net, rate } = calcNet(grossInput, category)

  return (
    <div className="flex-1 overflow-y-auto">

      {/* ── Hero — cabeçalho escuro com valor líquido em destaque ──── */}
      <div className="bg-gradient-header-dark px-5 pt-16 pb-6 rounded-b-[28px]">
        ...
        {/* ── Caixa de Valor Líquido — o número mais importante ──────
            Destaque máximo: tamanho 46px, cor Teal, glow. */}
      ...
      <div className="p-5 space-y-4">
        ...
        {/* ── Seletor de categoria ──────────────────────────────────────
            Trocar a categoria atualiza a taxa e o valor líquido em tempo real. */}
        <div>
            ...
            {(Object.keys(CAT_STYLES) as FreightCategory[]).map(cat => {
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl p-3 border-none cursor-pointer transition-all duration-250 ${
        ...
        {/* ── Breakdown financeiro completo ─────────────────────────────
            Transparência total: o motorista vê CADA valor separado. */}
        <div className="bg-white rounded-[18px] overflow-hidden shadow-card">
          ...
          {/* Linha: Taxa Muvv — variável por categoria */}
        ...
        {/* ── Conversão para Euro — só aparece em fretes ZPE ───────── */}
        {category === 'zpe' && (
